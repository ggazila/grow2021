...
class Solution2 {
    public static void main(String[] args) {
        System.out.println(new Solution2().isSumEqual("aaa", "a", "aaaa"));
        System.out.println(new Solution2().isSumEqual("aaa", "a", "aab"));
    }

    public boolean isSumEqual(String firstWord, String secondWord, String targetWord) {
        int first = Stream.of(firstWord.split(""))
                .map(s -> s.charAt(0) - 97)
                .map(String::valueOf)
                .reduce((s, s2) -> s + s2)
                .stream()
                ...
                .findFirst()
                .getAsInt();
        int second = Stream.of(secondWord.split(""))
                .map(s -> s.charAt(0) - 97)
                .map(String::valueOf)
                .reduce((s, s2) -> s + s2)
                .stream()
                ...
                .findFirst()
                .getAsInt();
        int target = Stream.of(targetWord.split(""))
                .map(s -> s.charAt(0) - 97)
                .map(String::valueOf)
                .reduce((s, s2) -> s + s2)
                .stream()
